...
from config import (
    DRONE_NAMES, DRONE_IPs, duration
)
...
class DroneControlPanel:
    ...
    def show_neighbours_info(self):
        ...
        neighbours, drone = self.get_neighbours_names()
        if neighbours:
            self.info_label.config(text=f"Neighbours of {drone}: {neighbours}", foreground="blue")
        else:
            self.info_label.config(text=f"{drone} has no neighbours in range.", foreground="blue")
    ...
    def get_neighbours_names(self):
        ...
        return neighbour_drones, drone_name
